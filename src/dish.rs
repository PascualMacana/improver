//! Plato de Petri: una célula se llena a medida que el cerebro encaja.
//! Es la imagen de `evolve`, sin escribir archivos.

use crate::{evolve_on, parse_expr, sse, target, Champion, BRAIN, XS};
use std::error::Error;
...
fn present(
    ...
    out.push_str("\x1b[H");
    out.push_str("\x1b[0m  mejorante  ·  petri dish\x1b[K\n");
    for y in 0..c.h {
        let mut last = 255_u8;
        for x in 0..c.w {
            let i = y * c.w + x;
